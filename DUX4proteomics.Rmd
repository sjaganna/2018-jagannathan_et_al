...
```{r}
#save(psmdata.static.filtered, file="data/rdata/psmdata.112017.Rdata")  
```  
...
```{r}
#save(psmdata.geneid.static, file="data/rdata/psmdata.geneid.static.112017.Rdata")
```
...
rnaseq <- foldchange.unfiltered[, c(2:7, 13:17)]

names(rnaseq) <- c("geneid", "iDUX4_logFC", "iDUX4_logCPM", "iDUX4_LR", "iDUX4_pval", "iDUX4_fdr", "vDUX4_logFC", "vDUX4_logCPM", "vDUX4_LR", "vDUX4_pval", "vDUX4_fdr")
``` 
...
```{r}
#save(data.rnaseq.silac.static, file="data/Rdata/data.rnaseq.silac.static_120117.Rdata")
#write.csv(data.rnaseq.silac.static, file = "data.rnaseq.silac.static_120117.csv")
...
#volcano plots
tmp <- data.rnaseq.silac.static %>% filter(rep !=3, rep != 4)
ggplot(tmp, aes(x=hl.ratio, y= -log10(pval))) +
geom_point(size=2, shape=21, col = "#10101090", bg = ifelse(tmp$pval < 0.05, "gold", "grey")) +
scale_x_continuous(name = "Normalized log2 (H/L) ratio",
breaks = seq(-5, 5, 1),
limits=c(-5, 5)) +
geom_vline(xintercept = 0, linetype="dashed") +
scale_y_continuous(name = "-log10 P value", limits=c(0, 50)) +
mytheme


#dux4 targets
tmp <- data.rnaseq.silac.static %>% filter(iDUX4_logFC > 1.5)
ggplot(tmp, aes(x=hl.ratio, y= -log10(pval))) + 
  geom_point(size=2, shape = 21, col = "black", bg = ifelse(tmp$pval < 0.05, "red", "grey")) + 
  scale_x_continuous(name = "Normalized log2 (H/L) ratio", 
                     breaks = seq(-5, 5, 1),
                     limits=c(-5, 5)) +
  scale_y_continuous(name = "-log10 P value", limits=c(0, 50)) +
...
# geom_violin(adjust=1.5,  trim = TRUE,
#            scale="width",
#            color="grey60",
#            fill = "#01010115",
#            colour = "#3366FF", cex=1) +
...
geom_jitter(aes(color = rep), size = 1) +
#geom_jitter(height = 0, color = "#FFD70080", size = 1) +
...
Fig 3: post-transcriptional regulation
```{r}

data <- data.rnaseq.silac.static %>% filter(rep != 3, rep != 4, pval < 0.05)

plot(data$iDUX4_logFC, data$hl.ratio, pch = 16, col = "#10101040", xlim = c(-10, 10), ylim = c(-10, 10))
text(5, 20, "rep != 3, rep != 4, pval<0.05")

```


Summary plot for individual genes across datasets
```{r}

x <- data.rnaseq.silac.static %>% select(geneid, iDUX4_logFC, vDUX4_logFC, hl.ratio.rep1, hl.ratio.rep2, hl.ratio)

names(x) <- c("geneid", "5-iDUX4_mRNA_14h",  "3-vDUX4_mRNA_36h", "2-vDUX4_protein_24h", "1-vDUX4_protein_36h", "4-iDUX4_protein_14h")
         
       x <- x %>% filter(geneid == "UPF1") %>% 
         gather(, value, 2:6)
       ...
       ggplot(x, aes(x = key, y = as.numeric(value))) + 
         ...
         xlab("") +
         #scale_y_continuous(limits=c(input$lim_left, input$lim_right)) +
         #ggtitle(geneid) +
         geom_bar(stat = "identity") +
         coord_flip()
```

Histogram plot for individual genes across SILAC datasets
```{r}
x <- psmdata.geneid.static$norm.hl.ratio

       # draw the histogram with the specified number of bins
       ggplot(psmdata.geneid.static %>% filter(geneid == "UPF1"), aes(norm.hl.ratio)) + 
         scale_x_continuous(limits=c(-10, 10)) +
         geom_histogram(bins = 50, fill = "#10101040") +
         mytheme +
         xlab("Normalized DUX4/Control ratio (log2)") +
         ylab("Peptide count") +
         facet_wrap(~rep, ncol =2, labeller = label_parsed) +
         #facet_wrap(~peptide.sequence) + 
         geom_vline(xintercept = 0, linetype="dashed")
       
```       


Gene-level plots for lists of genes
```{r}
...
# boxplot for representative genes
dataset = "psmdata.geneid.static"
i <- get(dataset)
...
genes <- c("RPL15", 
           #"NCBP1", "NCBP2","RBM8A", 
           #"MAGOH", "MAGOHB", "CASC3", "EIF4A3", "SAP18", 
           #"PNN", "ACIN1", "RNPS1", "PYM1", 
           #"ALYREF", "NXF1", "DDX39B", 
           #"NXF2", "NXF2B", "NXF3", "NXF5", "NXT1", "NXT2", 
           #"SRRM1", "RNMT", "RNGTT", 
           #"PABPN1", "PABPN1L", "BCL2L2-PABPN1", "MSI1", 
           #"MSI2", "DAZAP1", "PABPC1", "PABPC1L", "PABPC1L2A", 
           #"PABPC1L2B", "PABPC3", "PABPC4", "PABPC4L", "PABPC5", 
           "UPF1", "UPF2", "UPF3A", "UPF3B", 
           "SMG1", "SMG5", "SMG6", "SMG7", 
           #"PPP2CA",
           "XRN1", "XRN2" 
           #"NUDT16",
           #"DCP1A", "DCP1B", "DCP2", "DDX6", "EDC3", "EDC4", "PATL1"
           #"EXOSC1", "EXOSC2", "EXOSC3", "EXOSC4", "EXOSC5",
           #"EXOSC6", "EXOSC7", "EXOSC8", "EXOSC9"
           #"ETF1", "GSPT1", "GSPT2", "HBS1L", "PELO",
           #"PAPOLA", "PAPOLB", "PAPOLG", "CPSF6", "CPSF7",
           #"NUDT21", "CLP1", "PCF11", "CPSF1", "CPSF2",
           #"CPSF3", "CPSF4", "FIP1L1", "PPP1CA", "PPP1CB",
           #"PPP1CC", "WDR33", "WDR82", "SSU72", "CSTF1", 
           #"CSTF2", "CSTF2T", "CSTF3", "SYMPK"
           )

genes <- i[which(i$geneid %in% genes), ]

g <- ggplot(aes(y = norm.hl.ratio, x = geneid, fill = ""), data = genes)
g + xlab("") + ylab("log2 (DUX4/Ctrl)") +
  scale_x_discrete(limits = c("RPL15", 
                              #"NCBP1", "NCBP2","RBM8A", 
                              #"MAGOH", "MAGOHB", "CASC3", "EIF4A3", "SAP18", 
                              #"PNN", "ACIN1", "RNPS1", "PYM1", 
                              #"ALYREF", "NXF1", "DDX39B", 
  ...
  geom_boxplot(width= 1, outlier.shape=NA) +
  coord_flip() +
  scale_fill_manual(name = "", values = c("#10101040")) +
  geom_jitter(height = 0, color = "#10101020", size = 1) +
  theme(panel.background = element_blank(),
        panel.border = element_rect(fill=NA),
        panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank(),
